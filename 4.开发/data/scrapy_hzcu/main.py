...
def post_url_info__from_url(url):
    """
    :param url: 输入url
    :return: 网站url的Json格式
    """
    ...
    # 2.3. 图片链接
    print("  4. 关联图片：")
    company_model_type = "2.智谱AI-GLM4"
...
# 测试url
url = "http://www.hzcu.edu.cn/col/col10/index.html"
webiste_urls_json = post_url_info__from_url(url)

# 添加学校概况所涉及的第一级网站链接：
for i in range(len(webiste_urls_json)):
    print()
    print(str(i) + ". ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------")
    print(str(list(webiste_urls_json.keys())[i]) + ": " + str(list(webiste_urls_json.values())[i]))
    post_url_info__from_url(list(webiste_urls_json.values())[i])
