...
def api_login(phone_number, verification_code, password):
    ...
    :param password:
    :return: 失败返回0/失败信息，成功返回1/用户ID
    """
    url = f"http://{ip}:{port}/users/logins/users?phoneNumber={phone_number}&verificationCode={verification_code}&password={password}"
...
def api_register(phone_number, verification_code, password):
    ...
    :param verification_code:
    :param password:
    :return:失败返回0/失败信息，成功返回1/成功信息
    """
    #  post请求
    url = f"http://{ip}:{port}/users/logins/users"
    data = {
...
with col3:
    ...
    with tab1:
        ...
        with col11:
            # 登录按钮
            login_button = st.button("用户登录", key='button_1')
            if login_button:
                ...
                if is_success == 0:
                    ...
                    if 'user_id' not in st.session_state:
                        st.session_state['user_id'] = login_info
...
with col4:
    ...
    # st.link_button("管理员登录", "http://localhost:8502/")
